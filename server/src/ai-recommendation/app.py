...
def rank_masters(payload: Dict[str, Any]) -> List[int]:
    ...
    if not booked_master_ids:
        normalized_masters.sort(
            key=lambda m: (-m["rating"], -m["reviewCount"], m["id"])
        )
    ...
    for master in normalized_masters:
        ...
        score = (
            category_match * 10 +  
            master["rating"] * 6 +  
            min(10, master["reviewCount"] / 5) +  
            distance_score  
        )
        
        scored_masters.append({
            "id": master["id"],
            "score": score,
    ...
    scored_masters.sort(
        key=lambda m: (-m["score"], -m["rating"], -m["reviewCount"], m["id"])
    )
...
class RecommendationHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path == "/health":
            self._send_json(
                ...
                {
                    "statusCode": 200,
                    "message": "Python master recommendation service is running",
                    "data": {"status": "ok"},
                },
            )
            return
    ...
    def do_POST(self) -> None:
        ...
        self._send_json(
            ...
            {
                "statusCode": 200,
                "message": "Master recommendations calculated",
